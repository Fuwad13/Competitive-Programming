template <class T> struct segment_tree {
  ...
  T set_identity = T();
  segment_tree()
      : n(0), merge(nullptr), identity(0), add_identity(0), set_identity(0) {}
  segment_tree(int n, function<T(const T &, const T &)> merge, T identity,
               T add_identity, T set_identity)
      : n(n), merge(merge), identity(identity), add_identity(add_identity),
  ...
  segment_tree(const vector<T> &arr, function<T(const T &, const T &)> merge,
               T identity, T add_identity, T set_identity)
      : n(arr.size()), merge(merge), identity(identity),
        add_identity(add_identity), set_identity(set_identity) {
    stree.resize(4 * n + 5);
    lzadd.resize(4 * n + 5, add_identity);
    lzset.resize(4 * n + 5, set_identity);
    build(arr, 1, 0, n - 1);
