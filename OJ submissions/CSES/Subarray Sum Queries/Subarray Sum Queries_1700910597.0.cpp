...
node segTree[4*N];
  void build(int node, int b, int e){
    ...
    segTree[node].pref = max(segTree[left].pref, segTree[left].sum + segTree[right].pref);
    segTree[node].suf = max(segTree[right].suf, segTree[right].sum + segTree[left].suf);
    segTree[node].segMax = max(segTree[left].segMax, 
        max(segTree[right].segMax, segTree[left].suf + segTree[right].pref));
  }
 void update(int node, int ind, ll val, int b, int e){
  ...
  void solve(){
    ...
    build(1, 0, n-1);
    // for(int i = 1; i <=4*n; i++){
    //  cout << "node " << i << " >> " <<  segTree[i].raw << " " << segTree[i].segMax << " " << segTree[i].pref << " " << segTree[i].suf << ' ' << segTree[i].sum << endl;
    // }
