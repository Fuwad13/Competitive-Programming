...
int dy[]{1, 0, -1, 0}; // R U L D
 void solve(){
    ...
    for(int i = 0; i < n; i++){
        if(i==0 or i==n-1){
            for(int j = 0; j < m; j++){
                if(dista[i][j]!=-1 and (distm[i][j]==-1 or dista[i][j]<distm[i][j])){
                    boundary = {i, j};
                }
            }
        }
        else{
            if(dista[i][0]!=-1 and (distm[i][0]==-1 or dista[i][0]<distm[i][0])){
                boundary = {i, 0};
            }
            if(dista[i][m-1]!=-1 and (distm[i][m-1]==-1 or dista[i][m-1]<distm[i][m-1])){
                boundary = {i, m-1};
            }
         }
    }
